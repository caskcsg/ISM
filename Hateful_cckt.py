...
from models.model_cckt import ALBEF
...
def train(model, data_loader_hateful, data_loader_twitter, optimizer, tokenizer, warmup_steps, device, scheduler, mkmmd_loss, config, start_time, model_without_ddp, val_seen_loader_hateful, val_unseen_loader_hateful, test_seen_loader_hateful, test_unseen_loader_hateful, args):
    ...
    for i in range(0, config['schedular']['epochs']):
        ...
        if (i/850)>0 or not config['warm_up']:
            alpha = config['alpha']
        else:
        ...
        loss = Ly + 0.7 * mmd_loss

        if i % 500 == 0:
            print('i={}  Ly_target={:.5f}   Ly_source={:.5f}   mmd_loss={:.5f}   sum_loss={:.5f}'.format(i, Ly_target, Ly_source, mmd_loss, loss))
        ...
        if i%850==0 and i%step_size==0 and i<=config['schedular']['epochs']: 
...
if __name__ == '__main__':
    ...
    parser.add_argument('--config', default='/workspace/project/ALBEF/configs/Hateful_cckt.yaml')
