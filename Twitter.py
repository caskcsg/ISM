...
def evaluate(model, data_loader, tokenizer, device, config):
    ...
    pre_list = torch.cat(pre_list, 0) #[size, 2]
    label_list = torch.cat(label_list, 0) #[size]
    ...
    pre_list = pre_list.argmax(-1) #[size, 1]
    precision = precision_score(label_list.cpu().numpy(), pre_list.cpu().numpy())
    recall = recall_score(label_list.cpu().numpy(), pre_list.cpu().numpy())
    f1 = f1_score(label_list.cpu().numpy(), pre_list.cpu().numpy())
    # f1 = metrics.F1()(pre_list, label_list)
    # f1 = f1.item()
    # precision = metrics.Pre()(pre_list, label_list)
    # precision = precision.item()
    # recall = metrics.Recall()(pre_list, label_list)
    # recall = recall.item()
    ...
    # gather the stats from all processes
    metric_logger.synchronize_between_processes()
    print("Averaged stats: acc:", metric_logger.global_avg())   
...
def main(args, config):
        ...
        if utils.is_main_process():  
            if args.evaluate:
                ...
                if float(val_stats['acc'])>best:
                    ...
                    torch.save(save_obj, os.path.join(args.output_dir, 'checkpoint_best_acc.pth')) 
                    best = float(val_stats['acc'])
                    best_epoch=epoch

                if float(val_stats['f1'])>best_f1:
                    save_obj = {
                        'model': model_without_ddp.state_dict(),
                        'optimizer': optimizer.state_dict(),
                        'lr_scheduler': lr_scheduler.state_dict(),
                    ...
                    torch.save(save_obj, os.path.join(args.output_dir, 'checkpoint_best_f1.pth')) 
                    best_f1 = float(val_stats['f1'])
                    best_f1_epoch = epoch

                if float(val_stats['precision'])>best_precision:
                    save_obj = {
                        'model': model_without_ddp.state_dict(),
                        'optimizer': optimizer.state_dict(),
                        'lr_scheduler': lr_scheduler.state_dict(),
                    ...
                    torch.save(save_obj, os.path.join(args.output_dir, 'checkpoint_best_precision.pth')) 
                    best_precision = float(val_stats['precision'])
                    best_precision_epoch = epoch

                if float(val_stats['recall'])>best_recall:
                    save_obj = {
                        'model': model_without_ddp.state_dict(),
                        'optimizer': optimizer.state_dict(),
                        'lr_scheduler': lr_scheduler.state_dict(),
                    ...
                    torch.save(save_obj, os.path.join(args.output_dir, 'checkpoint_best_recall.pth')) 
                    best_recall = float(val_stats['recall'])
    ...
    if utils.is_main_process():   
        with open(os.path.join(args.output_dir, "log.txt"),"a") as f:
            f.write("best acc epoch: %d"%best_epoch)         
            f.write("best f1 epoch: %d"%best_f1_epoch)        
            f.write("best precision epoch: %d"%best_precision_epoch)
            f.write("best recall epoch: %d"%best_recall_epoch)       
...
if __name__ == '__main__':
    ...
    parser.add_argument('--output_dir', default='/workspace/project/ALBEF/output/Hateful')  
