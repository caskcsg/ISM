...
import sys
sys.path.append("../")
from Hateful_graph_final.model_multioff import ALBEF
...
def train(model, data_loader, optimizer, tokenizer, epoch, warmup_steps, device, scheduler, config):
    ...
    for i,(images, text, targets) in enumerate(metric_logger.log_every(data_loader, print_freq, header)):
        ...
        ly, utt_private_t, utt_private_v, utt_shared_t, utt_shared_v, kl_loss = model(images, text_inputs, targets=targets, train=True, alpha=alpha)  
...
def main(args, config):
    ...
    #需要更改
    datasets = create_dataset('multioff', config) 
    ...
    lr_scheduler, _ = create_scheduler(arg_sche, optimizer, flag=1)  
