...
def main(args, config):
    ...
    if args.distributed:
        ...
        samplers = create_sampler(datasets, [True, False, False, False, False], num_tasks, global_rank)         
    else:
        samplers = [None, None, None, None, None]

    train_loader, val_seen_loader, val_unseen_loader, test_seen_loader, test_unseen_loader = create_loader(datasets,samplers,
                                                          batch_size=[config['batch_size_train']]+[config['batch_size_test']]*4,
                                                          num_workers=[4,4,4,4,4],is_trains=[True,False,False,False,False], 
                                                          collate_fns=[None,None,None,None,None])
    ...
    for epoch in range(0, max_epoch):
        ...
        val_unseen_stats = evaluate(model, val_unseen_loader, tokenizer, device, config)
        test_seen_stats = evaluate(model, test_seen_loader, tokenizer, device, config)
        test_unseen_stats = evaluate(model, test_unseen_loader, tokenizer, device, config)
        ...
        if utils.is_main_process():  
            if args.evaluate:
                ...
                if float(test_unseen_stats['auc_roc'])>best_unseen_test_auc:
                    ...
                    best_unseen_test_auc_epoch = epoch
