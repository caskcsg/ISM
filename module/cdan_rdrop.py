...
from module.classifier import Classifier as ClassifierBase
from module.binary_accuracy import binary_accuracy
from module.grl import WarmStartGradientReverseLayer
from module.entropy import entropy
...
class ConditionalDomainAdversarialLoss(nn.Module):
    ...
    def forward(self, g_s: torch.Tensor, f_s: torch.Tensor, g_t: torch.Tensor, f_t: torch.Tensor) -> torch.Tensor:
        ...
        h_tmp = self.grl(self.map(f, g))
        h = torch.cat((h_tmp, h_tmp), 0)
        
        
        d = self.domain_discriminator(h)
        p, q = d.chunk(2, 0)
        kl_loss = compute_kl_loss(p, q)
        value, label = d.max(1)
        d = value.unsqueeze(1)
        d_label_tmp = torch.cat((
            torch.ones((g_s.size(0), 1)).to(g_s.device),
            torch.zeros((g_t.size(0), 1)).to(g_t.device),
        ))
        d_label = torch.cat((d_label_tmp, d_label_tmp), 0)
        weight_tmp = 1.0 + torch.exp(-entropy(g))
        weight = torch.cat((weight_tmp, weight_tmp), 0)
        batch_size = f.size(0) + f.size(0)
        weight = weight / torch.sum(weight) * batch_size
        self.domain_discriminator_accuracy = binary_accuracy(d, d_label) 
        bce_loss = self.bce(d, d_label, weight.view_as(d))
        #loss = bce_loss + 0.5 *  kl_loss 
        loss = bce_loss #+  2 *kl_loss
        return loss
