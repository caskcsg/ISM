...
class ALBEF(nn.Module):
    ...
    def forward(self, image, text, targets, alpha=0, train=True):
        ...
        if train:
            ...
            h_i = h_i @ self.proj_h
            t_i = t_i @ self.proj_t
            h_t, t_t = text_embed_t.chunk(2, 0)
            h_t = h_t @ self.text_projection_h
            t_t = t_t @ self.text_projection_t
            ...
            # logit_scale = self.logit_scale_h.exp()
            # h_logits_per_image = logit_scale * hi @ ht.t()
            # h_logits_per_text = h_logits_per_image.t()

            # logit_scale = self.logit_scale_t.exp()
            # t_logits_per_image = logit_scale * ti @ tt.t()
            # t_logits_per_text = t_logits_per_image.t()


            
            res = self.kl(hi, ht, ti, tt)
            print(res)
    ...
    def kl(self, h_i, h_t, t_i, t_t):
        ...
        print(p_loss_t, q_loss_h)
        if abs(kl_loss_h - kl_loss_t) >= 0.005:
            return np.repeat(10 * kl_loss_t.cpu().numpy(), 8)
        else :
            a = np.array([1, 1, 1, 1, 1, 1, 1, 1])
            return a
