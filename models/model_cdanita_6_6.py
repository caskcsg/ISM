...
class ALBEF(nn.Module):
    ...
    def forward(self, image, text, targets, alpha=0, train=True):
        ...
        if train:
            ...
            image_feat = F.normalize(self.vision_proj(image_hidden_state[5][:,0,:]),dim=-1) #256
            ###单一
            image_feat, _ = image_feat.chunk(2, 0)
            text_feat = F.normalize(self.text_proj(output.hidden_states[6][:,0,:]),dim=-1) #256
            ###单一
            text_feat, _ = text_feat.chunk(2, 0)
            ...
            if self.distill:                
                with torch.no_grad():
                    ...
                    image_feat_m = F.normalize(self.vision_proj_m(image_hidden_state_m[5][:,0,:]),dim=-1)  
                    ###单一
                    image_feat_m, _ = image_feat_m.chunk(2, 0)
                    ...
                    ###单一
                    text_feat_m, _ = text_feat_m.chunk(2, 0)
                ...
                a = F.log_softmax(sim_i2t, dim=1)
                b = F.log_softmax(sim_i2t, dim=1)*sim_i2t_targets
                c = torch.sum(F.log_softmax(sim_i2t, dim=1)*sim_i2t_targets,dim=1)
