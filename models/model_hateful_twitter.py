    ...
    def __init__(self,                 
        ...
        if self.distill:
            ...
            self.model_pairs = [[self.visual_encoder,self.visual_encoder_m],
                                ...
                                [self.cls_head_hateful,self.cls_head_m],
                               ]
            self.copy_params()        
            self.momentum = 0.995
    ...
    def forward(self, image, text, targets, alpha=0, train=True):
        ...
        if train:
            ...
            if self.distill:                
                ...
                loss = (1-alpha)*F.cross_entropy(head_hateful, targets) - alpha*torch.sum(
                    F.log_softmax(head_hateful, dim=1)*F.softmax(prediction_m, dim=1),dim=1).mean()
