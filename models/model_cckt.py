...
class ALBEF(nn.Module):
    def __init__(self,                 
        ...
        if self.distill:
            self.visual_encoder_m = VisionTransformer(
                img_size=config['image_res'], patch_size=16, embed_dim=768, depth=12, num_heads=12, 
                mlp_ratio=4, qkv_bias=True, norm_layer=partial(nn.LayerNorm, eps=1e-6))               
            self.text_encoder_m = BertModel.from_pretrained(text_encoder, config=bert_config, output_hidden_states=True, add_pooling_layer=False)      
    ...
    def forward(self, image, text, targets, alpha=0, train=True):
        ...
        if train:
            ...
            #hateful_image_embedding, twitter_image_embedding = image_embeds.last_hidden_state[:,0,:].chunk(2, 0)
            #hateful_text_embedding, twitter_text_embedding = output.hidden_states[:3:].chunk(2, 0)
            ...
            output = {
                        ...
                        #'hateful_image_embedding': hateful_image_embedding,
                        #'twitter_image_embedding': twitter_image_embedding,
                    }
            ...
            #prediction_twitter = self.cls_head_twitter(output.last_hidden_state[:,0,:]) 
              
            return prediction_hateful#,prediction_twitter
