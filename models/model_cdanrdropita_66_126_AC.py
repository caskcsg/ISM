...
class ALBEF(nn.Module):
    ...
    def forward(self, image, text, targets, alpha=0, train=True):
        ...
        if train:
            ...
            ###
            hateful_embed_image, twitter_embed_image = image_embed.chunk(2, 0)
            hateful_embed_text, twitter_embed_text = text_embed.chunk(2, 0)

            # hateful_image_feat = F.normalize(self.hateful_vision_proj(image_embed),dim=-1) #256
            # twitter_image_feat = F.normalize(self.twitter_vision_proj(image_embed),dim=-1) #256
            # #fusion
            ###
            ...
            # hateful_text_feat = F.normalize(self.hateful_text_proj(output.hidden_states[6][:,0,:]),dim=-1) #256
            # twitter_text_feat = F.normalize(self.twitter_text_proj(output.hidden_states[6][:,0,:]),dim=-1) #256
            ...
            with torch.no_grad():
                ...
                # hateful_image_feat_m = F.normalize(self.hateful_vision_proj_m(image_embeds_m[:,0,:]),dim=-1)  
                # twitter_image_feat_m = F.normalize(self.twitter_vision_proj_m(image_embeds_m[:,0,:]),dim=-1)  
                ...
                # hateful_text_feat_m = F.normalize(self.hateful_text_proj_m(text_output_m.last_hidden_state[:,0,:]),dim=-1) 
                # twitter_text_feat_m = F.normalize(self.twitter_text_proj_m(text_output_m.last_hidden_state[:,0,:]),dim=-1) 
                ...
                # hateful_sim_i2t_m = hateful_image_feat_m_hateful @ hateful_text_feat_all / self.temp 
                # hateful_sim_t2i_m = hateful_text_feat_m_hateful @ hateful_image_feat_all / self.temp 

                # twitter_sim_i2t_m = twitter_image_feat_m_twitter @ twitter_text_feat_all / self.temp 
                # twitter_sim_t2i_m = twitter_text_feat_m_twitter @ twitter_image_feat_all / self.temp
                ...
                # hatefuL_sim_i2t = hateful_image_feat_hateful @ hateful_text_feat_all / self.temp 
                # hateful_sim_t2i = hateful_text_feat_hateful @ hateful_image_feat_all / self.temp 

                # twitter_sim_i2t = twitter_image_feat_twitter @ twitter_text_feat_all / self.temp 
                # twitter_sim_t2i = twitter_text_feat_twitter @ twitter_image_feat_all / self.temp 
                ...
                # hateful_loss_ita = (hateful_loss_i2t+hateful_loss_t2i)/2  
                # twitter_loss_ita = (twitter_loss_i2t+twitter_loss_t2i)/2  
                # ##fusion
                ####
            ...
            ####
            # self._dequeue_and_enqueue_hateful(hateful_image_feat_m_hateful, hateful_text_feat_m_hateful)
            # self._dequeue_and_enqueue_twitter(twitter_image_feat_m_twitter, twitter_text_feat_m_twitter)
