...
from models.model_cdan import ALBEF
...
#from module.dann import DomainAdversarialLoss, ImageClassifier
...
#.cuda() 很关键
discriminator = DomainDiscriminator(in_feature=768 * 2, hidden_size=768).cuda()
cdan_loss = ConditionalDomainAdversarialLoss(discriminator, reduction='mean').cuda()

def cl_w(y_softmax, label, d):
    the_index = torch.LongTensor(np.array(range(y_softmax.shape[0]))).cuda()
...
def train(model, data_loader_hateful, data_loader_twitter, optimizer, tokenizer, warmup_steps, device, scheduler, cdan_loss, config, start_time, model_without_ddp, val_seen_loader_hateful, val_unseen_loader_hateful, test_seen_loader_hateful, test_unseen_loader_hateful, args):
    ...
    for i in range(0, config['schedular']['epochs']):
        ...
        #hateful_image_embedding, twitter_image_embedding = hateful_image_embedding.to(device,non_blocking=True), twitter_image_embedding.to(device,non_blocking=True)
        #hateful_text_embedding, twitter_text_embedding = hateful_text_embedding.to(device,non_blocking=True), twitter_text_embedding.to(device,non_blocking=True)
        #mmd_loss = mkmmd_loss(hateful_hidden_state, twitter_hidden_state)

        #adv_loss :(source, target)        
        ...
        cdan_loss_image = cdan_loss(twitter_head_hateful, twitter_image_embedding, hateful_head_hateful, hateful_image_embedding)
        cdan_loss_text = cdan_loss(twitter_head_hateful, twitter_text_embedding, hateful_head_hateful, hateful_text_embedding)
        Ld = cdan_loss_image + cdan_loss_text
        ...
        # print(loss)
        if i % 500 == 0:
            print('i={}  Ly_target={:.5f}   Ly_source={:.5f}   Ld={:.5f}   sum_loss={:.5f}'.format(i, Ly_target, Ly_source, Ld, loss))
...
def main(args, config):
    ...
    if not args.evaluate:
        ...
        train(model, train_loader_hateful, train_loader_twitter, optimizer, tokenizer,  warmup_steps, device, lr_scheduler, cdan_loss, config, start_time, model_without_ddp, val_seen_loader_hateful, val_unseen_loader_hateful, test_seen_loader_hateful, test_unseen_loader_hateful, args)
        #train_stats = 
